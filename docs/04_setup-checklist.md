...
### さらに安全にしたい場合: プリペイドカード
- Google の「前払い(手動支払い)」は**日本の新規アカウントでは選択不可**。
- 代わりに **プリペイドカード(Kyash 等)に¥10,000だけチャージして登録** すれば、
  残高以上は物理的に課金されない=前払いと同じ安心。Kyashはバーチャルカード即発行可。
- 注意: 稀にプリペイドが与信で弾かれる/失敗した請求が少額残ることがある。その場合は
  通常クレカ+上記1〜3の終了日運用に切り替える。
